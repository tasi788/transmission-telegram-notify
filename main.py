...
def initlogger(logname,level='info'):
	...
	formatter = logging.Formatter('[%(asctime)s] {levelname} %(name)s %(message)s'.format(levelname='%(levelname)s'),datefmt='%Y/%m/%d %I:%M:%S %p')
...
def run():
	...
	def notify(text):
		url = 'https://api.telegram.org/bot{token}/sendMessage'.format(token=config.telegram.token)
